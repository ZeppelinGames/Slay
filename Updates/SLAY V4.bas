...
SUB CreateNewSave
    ...
    DO
        _LIMIT 60
        LOCATE (h / 2 + 15) / fh + hf, (w / 2 - 50) / fw + fw: INPUT "", name$
...
SUB DrawMenu
    ...
    scaledText w / 2, h / 10 * 5.25, 25, _RGBA(255, 255, 255, 255), "OPTIONS"
    scaledText w / 2, h / 10 * 5.75, 25, _RGBA(255, 255, 255, 255), "QUIT"
        ...
        SELECT CASE INKEY$
            ...
            CASE CHR$(13) 'ENTER PRESSED
                IF opt = 0 THEN 'GAME INFO
                    LINE (0, h / 10 * 4.6)-(w, h), col, BF 'CLEAR OPTIONS
...
SUB Infinite
    ...
    DO
        ...
        sTime = TIMER(0.001)
        'Spawn enemy
        IF TIMER - prevTime > spawnTime THEN
            prevTime = TIMER
            spawnAmount = INT(spawnedEnemies / 50) + 1
            IF spawnAmount > 4 THEN
                spawnAmount = 4
            END IF

            FOR x = 1 TO spawnAmount
                enemyIndex = -1
                FOR n = 1 TO 100
                    IF darts(n).alive = 0 THEN
                        enemyIndex = n
                    END IF
                NEXT n

                IF NOT enemyIndex = -1 THEN

                    x = INT(RND * 2)
                    IF x = 1 THEN
                        darts(enemyIndex).x = _WIDTH - 50
                        darts(enemyIndex).speed = -1
                    ELSE
                        darts(enemyIndex).x = 50
                        darts(enemyIndex).speed = 1
                    END IF
                    darts(enemyIndex).y = INT(RND * (_HEIGHT / 10 * 8 - 50)) + 50

                    darts(enemyIndex).alive = 1
                    spawnedEnemies = spawnedEnemies + 1

                    IF spawnTime > 0.2 THEN
        ...
        FOR n = 1 TO 100
            col = CheckCollision(dartImg&, sword&, darts(n).x, darts(n).y, swordX, swordY)
            IF col = 1 THEN
                darts(n).alive = 0
                score = score + INT((_HEIGHT / (_HEIGHT - darts(n).y)))
            ...
            col = CheckCollision(dartImg&, player&, darts(n).x, darts(n).y, pX, pY)
            IF col = 1 THEN
                health = 0
                gameover = 1
            ...
            IF darts(n).alive = 1 THEN
                ...
                darts(n).x = darts(n).x + (darts(n).speed * enemySpeed)

                IF darts(n).x < 1 THEN
                    darts(n).alive = 0
                END IF
                IF darts(n).x > _WIDTH - 1 THEN
                    darts(n).alive = 0
                END IF

                IF darts(n).y < 50 THEN
                    darts(n).y = 50
                END IF

                IF darts(n).y > _HEIGHT - 50 THEN
                    darts(n).y = _HEIGHT - 50
                END IF

                IF darts(n).speed = 1 THEN
                    RotoZoom2 darts(n).x, darts(n).y, dartImg&, 1, 1, 0
                ELSE
                    RotoZoom2 darts(n).x, darts(n).y, dartImg&, 1, 1, 180
                END IF
            END IF
        ...
        scaledText _WIDTH / 2, _HEIGHT / 10, 50, _RGB(255, 255, 255), STR$(score)
        IF gameover = 1 THEN
            scaledText _WIDTH / 2, _HEIGHT / 2, 75, _RGB(255, 255, 255), "GAME-OVER"
        END IF
        _DISPLAY
